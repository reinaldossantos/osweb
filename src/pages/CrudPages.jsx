...
export function PageFuncionarios() {
  ...
  return (
    <>
      ...
      {showModal && (
        <Modal
          title={editItem ? "Editar Funcionário" : "Novo Funcionário"}
          onClose={() => setShowModal(false)}
        >
          <FuncionariosForm
            item={editItem}
    ...
    <div>
      ...
      >
        <div style={{ display: "flex", gap: 24, fontSize: 13 }}>
          ...
          <span>
            <strong>Valor Base:</strong> {fmt(servico.valor_base)}
          </span>
          {servico.descricao && (
            <span>
          ...
          <tbody>
            {etapas.map((etapa, i) => (
              ...
              >
                ...
                <td style={{ padding: "12px 16px", color: "#6B7280" }}>
                  {etapa.duracao_estimada_horas
                    ? `${etapa.duracao_estimada_horas}h`
                    : "—"}
                </td>
              </tr>
            ))}
...
export function PageServicos() {
  ...
  return (
    <>
      ...
      {showModal && (
        <Modal
          title={editItem ? "Editar Serviço" : "Novo Serviço"}
          onClose={() => setShowModal(false)}
        >
          <ServicosForm
            item={editItem}
      ...
      {viewEtapas && (
        <Modal
          title={`Etapas — ${viewEtapas.nome}`}
          onClose={() => setViewEtapas(null)}
          size="md"
        >
          <EtapasDoServico
            servico={viewEtapas}
...
export function PageEtapas() {
  ...
  return (
    <>
      ...
      {showModal && (
        <Modal
          title={editItem ? "Editar Etapa" : "Nova Etapa"}
          onClose={() => setShowModal(false)}
        >
          <EtapasForm
            item={editItem}
...
export function PageTiposOS() {
  ...
  return (
    <>
      ...
      {showModal && (
        <Modal
          title={editItem ? "Editar Tipo OS" : "Novo Tipo OS"}
          onClose={() => setShowModal(false)}
        >
          <TiposOSForm
            item={editItem}
...
export function PageUsuarios() {
  ...
  const filtered = items.filter(
    (i) =>
      !busca ||
      i.funcionarios?.nome?.toLowerCase().includes(busca.toLowerCase()),
  );
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((item, i) => (
                ...
                >
                  ...
                  <td style={{ padding: "12px 16px", color: "#6B7280" }}>
                    {item.funcionarios?.email || "—"}
                  </td>
                  <td style={{ padding: "12px 16px" }}>
                    <span
                      style={{
                        background:
                          item.perfil === "admin" ? "#FEF3C7" : "#F5F3FF",
                        color: item.perfil === "admin" ? "#92400E" : "#6D28D9",
                        padding: "2px 8px",
                        borderRadius: 6,
                        fontSize: 12,
                        fontWeight: 600,
