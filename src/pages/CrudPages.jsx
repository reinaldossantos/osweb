...
export function PageFuncionarios() {
  ...
  async function load() {
    const { data } = await supabase.from("funcionarios").select("*, cargos(nome)").eq("ativo", true).order("nome");
    setItems(data || []);
  }
...
export function PageCargos() {
  ...
  async function load() {
    const { data } = await supabase.from("cargos").select("*").eq("ativo", true).order("nome");
    setItems(data || []);
  }
...
export function PageEtapas() {
  ...
  async function load() {
    const { data } = await supabase.from("etapas_servico").select("*, servicos(nome)").eq("ativo", true).order("ordem");
    setItems(data || []);
  }
...
export function PageTiposOS() {
  ...
  async function load() {
    const { data } = await supabase.from("tipos_os").select("*").eq("ativo", true).order("codigo");
    setItems(data || []);
  }
